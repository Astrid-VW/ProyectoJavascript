...
const addListeners = () => {
  ...
  buttonLogin.addEventListener("click", (e) => {
    ...
    // como trabajo con el localstorage lo convierto a string
     ...
     * y así lo asocia para coger los me gusta de los pokemon
     */
    if (localStorage.getItem(`${valueInput}USER`)) {
      const localUser = localStorage.getItem(`${valueInput}USER`);
      const parseUser = JSON.parse(localUser);

      // importante es que si me logueo, poner el token de autenticacion como true
      ...
      // y llamamos a la funcion de set del user logado actual
       ...
       * de cero con su array de fav vacio y el token en true y el nombre es el nombre del input
       */
      const customUser = {
        name: username.value,
        fav: [],
      ...
      // seteamos el nombre en su estado asi como los datos del usuario en el segundo estado customUser
      setUser(`${valueInput}USER`);
